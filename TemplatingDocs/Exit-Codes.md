...
Applicable as well if template option [marked as required](Reference-for-template.json#isrequired) was not passed during the template instantiation.
...
Applicable to a case when post action fails - unless it is configured to [continue on errors](Post-Action-Registry#continueOnError).
